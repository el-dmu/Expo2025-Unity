...
public class RightHandDataTracker : MonoBehaviour
{
    [Header("오른손 모델 연결")]
    [Tooltip("Hierarchy 뷰에 있는 오른손 모델(예: CapsuleHand_R)을 여기에 연결해주세요.")]
    ...
    void Update()
    {
        ...
        if (timer <= 0f)
        {
            ...
            // rightHandModel이 없거나 손이 추적되고 있지 않으면 아무것도 하지 않음
            ...
            Leap.Hand hand = rightHandModel.GetLeapHand();
            ...
            handDataStringBuilder.AppendFormat("오른손 회전(X,Y,Z): ({0:F1}, {1:F1}, {2:F1}) | ", rot.x, rot.y, rot.z);

            // 2. 손가락 구부림 값을 '엄지, 검지, 중지, 약지, 소지' 순서로 추가
            handDataStringBuilder.AppendFormat("오른손 구부림(엄,검,중,약,소): ({0:F1}, {1:F1}, {2:F1}, {3:F1}, {4:F1})",
