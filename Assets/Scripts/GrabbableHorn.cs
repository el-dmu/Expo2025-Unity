...
{
    [Header("상태 제어")]
    [Tooltip("이 오브젝트를 잡을 수 있는지 여부를 외부에서 제어합니다.")]
    public bool isGrabbable = false; // ★ 외부 제어용 스위치 추가
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.GetComponentInParent<HandModelBase>() != null)
        {
            ...
            if (isGrabbable && !isGrabbed && highlightMaterial != null)
            {
    ...
    private void OnTriggerExit(Collider other)
    {
        if (other.GetComponentInParent<HandModelBase>() != null)
        {
            ...
            if (isGrabbable && !isGrabbed && originalMaterial != null)
            {
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        if (other.GetComponentInParent<HandModelBase>() != null)
        {
            ...
            {
                if (hand.GrabStrength > 0.8f && isGrabbable)
                {
                    GrabHorn(hand);
                    break;
                }
    ...
    private void GrabHorn(Hand hand)
    {
        ...
        if (isGrabbable && originalMaterial != null)
        {
            hornRenderer.material = originalMaterial;
        }
